                    ...
                    <div class="tab-pane fade show active" id="medical" role="tabpanel">
                        ...
                        @foreach (['subjective', 'objective', 'assessment', 'plan'] as $field)
                            <div class="mb-3">
                                ...
                                <textarea name="{{ $field }}" class="form-control @error($field) is-invalid @enderror" rows="3"
                                    {{ $isReadonly ? 'disabled' : 'required' }}>{{ old($field, $record->$field ?? '') }}</textarea>
                                @error($field)
                            ...
                            @forelse($appointment->services as $appService)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        @if (!$isReadonly)
                                            <button type="button"
                                                class="btn btn-sm btn-outline-danger ms-2 delete-service-btn"
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                ...
                                error: () => {
                                    Swal.fire('Error', 'Gagal menghapus layanan.',
                                        'error');
                                }
                            });
                        }
